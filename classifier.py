...
class CLASSIFIER:
    def __init__(
        ...
        self.all_indices = (
            torch.linspace(0, self.ntrain - 1, self.ntrain).long().to(self.device)
        )
                ...
                if generalized:
                    ...
                    with torch.no_grad():
                        ...
                        acc_novel = self.val_gzsl(
                            self.test_novel_feature,
                            self.test_novel_label,
                            self.novelclasses,
                        )
                    ...
                    if H > best_H:
                        ...
                        best_H = H
                    self.acc_seen, self.acc_novel, self.H = (
                        best_seen,
                        best_novel,
                        best_H,
                    )
    ...
    def val(self, test_X, test_label, target_classes):
        ...
        acc = self.compute_per_class_acc(
            map_label(test_label, target_classes),
            predicted_label,
    ...
    def compute_per_class_acc(self, test_label, predicted_label, nclass):
        ...
        target_classes = torch.arange(0, nclass, out=torch.LongTensor()).to(
            self.device
        )  # changed from 200 to nclass on 24.06.
        predicted_label = predicted_label.to(self.device)
